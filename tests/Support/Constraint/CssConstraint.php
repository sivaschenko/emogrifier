...
abstract class CssConstraint extends Constraint
{
    ...
    private const CSS_REGULAR_EXPRESSIOM_PATTERN = '/
        (?<![\\s;}])\\s*+(\\})\\s*+     # `}`, captured in group 1, with possible whitespace either side, not preceded
                                        # by `;` or `}`
        |\\s*+([{};:,])\\s*+            # `{`, `}`, `;`, `:` or `,`, captured in group 2, with possible whitespace
                                        # either side
        |(^\\s++)                       # whitespace at the very start, captured in group 3
        |(>)\\s*+                       # `>` (e.g. closing a `<style>` element opening tag), captured in group 4, with
                                        # optional whitespace following
    ...
    protected static function getCssNeedleRegularExpressionPattern(string $needle): string
    {
        ...
        $needleMatcherAllowingQuotedUrl = \preg_replace(
            '/url\\\\\\(((?:[^"\'\\s\\\\]|\\\\[^\\)])++)\\\\\\)/',
            'url\\("?+$1"?+\\)',
            $needleMatcher
        );
        $needleMatcherAllowingQuotedUrlOrMinifiedNumbers = \preg_replace(
            ['/(\\.[1-9]++)0++(?!\\d)/', '/(\\d)\\\\\\.0++(?!\\d)/', '/(?<!\\d)0(\\\\\\.\\d)/'],
            ['${1}0*+', '$1(?:\\\\.0++)?+', '0?+$1'],
            $needleMatcherAllowingQuotedUrl
        );

        return '/' . $needleMatcherAllowingQuotedUrlOrMinifiedNumbers . '/';
    }
    ...
    private static function getCssNeedleRegularExpressionReplacement(array $matches): string
    {
        switch (true) {
            ...
            case isset($matches[2]) && $matches[2] !== '':
                return '\\s*+' . \preg_quote($matches[2], '/') . '\\s*+';
            case isset($matches[3]) && $matches[3] !== '':
                return '\\s*+';
            case isset($matches[4]) && $matches[4] !== '':
                return \preg_quote($matches[4], '/') . '\\s*+';
            case isset($matches[5]) && $matches[5] !== '':
                return '\\s++';
            default:
